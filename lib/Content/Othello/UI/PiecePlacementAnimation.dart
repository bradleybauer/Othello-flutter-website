...
class PieceConversionAnimationW extends AnimatedWidget {
  ...
  PieceConversionAnimationW(
    ...
    final Color startColor =
        player == Othello.WHITE ? Colors.blueGrey[400]! : Colors.amber[400]!;
    final Color startShade =
        player == Othello.WHITE ? Colors.blueGrey[600]! : Colors.amber[600]!;
    final Color endColor =
        player == Othello.WHITE ? Colors.amber[400]! : Colors.blueGrey[400]!;
    final Color endShade =
        player == Othello.WHITE ? Colors.amber[600]! : Colors.blueGrey[600]!;
    color =
        ColorTween(begin: startColor, end: endColor).animate(CurvedAnimation(
      parent: driver,
      curve: Interval(t, 1, curve: Curves.linear),
    ));
    shade =
        ColorTween(begin: startShade, end: endShade).animate(CurvedAnimation(
      parent: driver,
      curve: Interval(t, 1, curve: Curves.linear),
    ));
  ...
  late Animation<double> driver;
  late Animation<double> bounce;
  late Animation<Color?> color;
  late Animation<Color?> shade;
  ...
  Widget build(BuildContext context) {
    ...
    double jump = 1 - t * t;
    if (bounce.value > .5) {
      jump = (1 - math.pow(t * t, 2)).toDouble();
    }
    ...
    return Stack(alignment: Alignment.center, children: [
      Container(
        transform:
            Matrix4.translationValues(0, -.25 * brickEdgeLength * jump, 0),
        child: BoardView.positionPiece(i, j, brickEdgeLength,
            Piece(color.value!, shade.value!, brickEdgeLength)),
      )
    ]);
  }
}
...
class PiecePlacementAnimationW extends AnimatedWidget {
  ...
  PiecePlacementAnimationW(
      ...
      required this.piece,
      required this.brickEdgeLength})
      : drop = Tween<double>(begin: -30 * brickEdgeLength, end: 0)
        ...
        child: Opacity(
            ...
            child: Transform(
              ...
              transform: Matrix4.translationValues(
                      (i + j - 7) * brickEdgeLength / 2,
                      .5 * (i - j) * brickEdgeLength / 2,
                      1) *
...
class PiecePlacementAnimation extends StatefulWidget {
  ...
  @override
  _PiecePlacementAnimationState createState() =>
      _PiecePlacementAnimationState();
}

class _PiecePlacementAnimationState extends State<PiecePlacementAnimation>
    with TickerProviderStateMixin {
  static const int delay = 100;
  late AnimationController _controller;
  late List<Animation<double>> placementAnimations;
  late List<Animation<double>> conversionAnimations;
  ...
  void initState() {
    ...
    final int numMilliseconds =
        1000 + numPlacementMilliseconds + numConversionMilliseconds;

    _controller = AnimationController(
        duration: Duration(milliseconds: numMilliseconds), vsync: this);
    _controller.forward();
  }
  ...
  Widget build(BuildContext context) {
    ...
    for (int i = 0; i < widget.placedPieces.length; i++) {
      placementAnimations.add(CurvedAnimation(
        parent: _controller,
        curve: Interval(
            i * delay / numMilliseconds, (i * delay + 1000) / numMilliseconds,
            curve: Curves.linear),
      ));
    }
    conversionAnimations = <Animation<double>>[];
    for (int i = 0; i < widget.conversions.length; i++) {
      final double dist = math.sqrt(math.pow(
              widget.conversions[i][0] - widget.placedPositions[0][0], 2) +
          math.pow(widget.conversions[i][1] - widget.placedPositions[0][1], 2));
      conversionAnimations.add(CurvedAnimation(
        parent: _controller,
        curve: Interval(
            (numPlacementMilliseconds + dist * delay) / numMilliseconds,
            math.min(
                1,
                (numPlacementMilliseconds + dist * delay + 1000) /
                    numMilliseconds),
            curve: Curves.linear),
      ));
    }
