  ...
  Widget build(BuildContext context) {
    ...
    for (int i = 0; i < 8; i++) {
      ...
      for (int j = 0; j < 8; j++) {
        ...
        double y = -.75 * brickDiagonalRadius * (.5 - .5 * math.cos(t * math.pi * 2.0)) - brickDiagonalRadius;
        if (showFinishedAnimation) {
          t = math.min(1, _progress.value * 2 + (i + j) / 16.0);
          y = -.75 * brickDiagonalRadius * (.5 - .5 * math.cos(t * math.pi * 2.0)) -
              (1 - Curves.bounceOut.transform(_progress.value)) * brickDiagonalRadius;
        }
        result[i].add(Transform(transform: Matrix4.translationValues(0, y, 0), child: bricks[i][j]));
      }
    }
...
class BoardLoadingAnimation extends StatefulWidget {
  const BoardLoadingAnimation(
      {this.game, this.brickDiagonalRadius, this.bricks, this.animationsCompleteCallback, this.boardBase});
...
class _BoardLoadingAnimationState extends State<BoardLoadingAnimation> with TickerProviderStateMixin {
  AnimationController _loadingController;
  AnimationController _finishedController;
  ...
  void initState() {
    super.initState();
    _finishedController = AnimationController(duration: const Duration(milliseconds: 750), vsync: this);
    _loadingController = AnimationController(duration: const Duration(seconds: 3), vsync: this)
      ..forward()
  ...
  Widget build(BuildContext context) {
    return BoardLoadingAnimationW(
      controller: readyToShowFinishedAnimation ? _finishedController : _loadingController,
